...
echo "[+] Janji Jiwa Account Creator - By: Andrea Zakharia- Piano\n";
echo "[+] Nomer HP: $nope\n";
echo "[+] PIN: 121212\n";
		...
		if (strpos($ver[1], '"status":"success"')) {
			...
			if (strpos($chg[1], '"status":"success"')) {
				...
				if (strpos($tkn[1], '"access_token"')) {
					$cfg = curl('https://api.jiwa.app/api/users/profile/update', '{"address":"","birthday":"","celebrate":"","email":"'.strtolower(trim($na[0])).mt_rand(10,9999).'@v7ecub.com","gender":"","id_city":"","job":"","name":"'.trim($na[0]).'","phone":"'.$nope.'","pin":"'.$pin.'"}', $headers);
